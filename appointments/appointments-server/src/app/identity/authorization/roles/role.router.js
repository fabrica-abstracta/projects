...
router.post(
  ...
  async (req, res) => {
    if (req.body.id) {
      ...
      if (!oldRecord) {
        return res.status(404).json({
          message: "El role solicitado no fue encontrado.",
        });
      }

      if (oldRecord?.status && [].includes(oldRecord.status)) {
        return res.status(409).json({
          message: "El role no puede ser modificado por su estado actual.",
        });
      }
      ...
      return res.status(200).json({
        message: "El role fue actualizado exitosamente.",
      });
    }
    ...
    return res.status(200).json({
      id: String(record._id),
      message: "El role fue creado exitosamente.",
    });
  },
);
...
router.delete(
  ...
  async (req, res) => {
    ...
    return res.status(200).json({
      message: "El role fue eliminado exitosamente.",
    });
  },
);
